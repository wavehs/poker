...
## 2024-05-18 - Monte Carlo Hand Filtering
**Learning:** In hot Monte Carlo loops, replacing `random.sample` with a `while` loop rejection sampling over a pre-allocated boolean `forbidden` array is significantly faster for drawing a small number of items from a fixed set (like a card deck).
**Action:** When filtering known used cards, avoid dynamic array creations or set operations inside the hot loop; use a pre-allocated boolean array toggled by index.
