...
class EquitySolver:
    ...
    def compute_range_vs_range_equity(
        ...
        # ⚡ Bolt: Extract instance method reference outside the loop to reduce lookup overhead
        ...
        for _ in range(sims):
            ...
            # Draw board
            # ⚡ Bolt Optimization: Use random.sample over a dynamically filtered deck.
            tmp_deck = [c for c in deck if c != v_hand[0] and c != v_hand[1]]
            ...
            # ⚡ Bolt Optimization: Use pre-allocated O(1) array instead of sets in hot loop.
