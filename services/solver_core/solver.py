...
class EquitySolver:
    ...
    def compute_range_vs_range_equity(
        ...
        # ⚡ Bolt: Extract evaluate method to avoid attribute lookups in hot loop
        ...
        for _ in range(sims):
            ...
            # Evaluate villain
            # ⚡ Bolt: Avoid list() constructor by explicit unpacking
            v_rank = evaluate([v0, v1] + full_board)

            # Construct O(1) lookup for forbidden cards to optimize hero hand filtering
            # ⚡ Bolt Optimization: Set true on the pre-allocated array instead of allocating sets
            forbidden[v0] = True
            forbidden[v1] = True
