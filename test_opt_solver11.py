...
class FastSolver(EquitySolver):
    def compute_range_vs_range_equity(self, hero_range_cards, villain_range_cards, community_cards, simulations=None):
        ...
        for _ in range(sims):
            ...
            # Look at memory:
            # "In performance-critical Monte Carlo inner loops (such as compute_range_vs_range_equity), constructing a single O(1) forbidden set outside the loop for card validation is up to ~4x faster than performing repeated list or tuple membership checks inside the hot loop."
...
def bench_range():
    ...
    print("Time taken for 1000 sims (forbidden set from memory):", time.time() - t0)
